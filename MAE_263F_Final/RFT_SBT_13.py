...
def getFh(q, q_old, dt, R, nv): # Get hydrodynamic forces from RFT
    """
...
def getf(q, q_old, dt, mu, deltaL, R, nv):
    epsilon = deltaL / 2
    ...
    for j in range(nv):
        ...
        for n in range(nv):
            ...
            r = np.array([xj - xn, yj - yn]) # [2N]
            Sij = RegGreenFunction(r, epsilon) # [2 x 2]
            Sij_bar[2 * j:2 * j + 2, 2 * n:2 * n + 2] = Sij
            
            # Solve for force contribution
    Fregs = -np.linalg.solve(Sij_bar, velocities) * deltaL * (8 * np.pi * mu)
    #Fregs = f_contribution * deltaL /  8 * np.pi * mu # Accumulate forces
    return Fregs
...
## Need If statement for using RFT vs SBT and to compute SBT using equation 16 from helicoil PNAS paper
        ...
        if SBTFlag == 0:
            Fhydro, Jhydro= getFh(q_new, q_old, dt, R, nv)
        elif SBTFlag == 1:
            #Fhydro, Jhydro = getFh_stokeslet(q_new, q_old, dt, C_t, C_n, nv, mu, deltaL/2)
            Fhydro = getf(q_new, q_old, dt, mu, deltaL, R, nv)
            Jhydro = np.zeros([len(q), len(q)])
        else:
...
# Densities
rho_metal = 1000*(RodLength*np.pi*ro**2) / (Vtotal)
rho_gl = 0.0
...
if SBTFlag == 0:
    suff = f'RFTv4_RL{RodLength}_mu{mu}_N{nv}_dt{dt}_HA{head_amplitude}_HF{head_frequency}_un'
elif SBTFlag == 1:
    suff = f'SBTv4_RL{RodLength}_mu{mu}_N{nv}_dt{dt}_HA{head_amplitude}_HF{head_frequency}_un'
else:
